...
**None active** — **Phase 1 underway.** Last completed: **Sprint 1.2 — Commerce Core (Pricing + Inventory)** (validated). Awaiting approval to begin Sprint 1.3.
...
**Sprint 1.3 — Cart** (per the roadmap). Awaiting approval. (Deferred, picked up when a Docker host is available: Prisma persistence + per-context Postgres schemas, S3/MinIO for Media, Redpanda/Debezium broker wiring, HTTP/GraphQL + gRPC transport, audit/Ory/Keto, flag rollout engine.)
